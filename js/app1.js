$(document).ready(function () {
    ...
    $('.inspiration-getter').submit(function (event) {
        ...
        $('.results').html('');
        //get the value of the tags the user submitted
        var tag = $(this).find("input[name='answerers']").val(); //?? 'answerers' vs 'tags' why doesn't match var name
        //run the API search with the user input above
        getInspiration(tag);
    });
});
...
var getUnanswered = function (tags) {
    ...
    $.ajax({
        ...
        .done(function (result) { //this waits for the ajax to return with a succesful promise object
            ...
            $('.searh-results').html(searchResults);
            //$.each is a higher order function. It takes an array and a function as an argument.
            //the funtion is executed once for each item in the array.
            $.each(result.items, function (i, item) {
                //call the folling function (showQuestions(item)) to show the API results
...
//this function takes the question object returned by the StackOverflow request and returns a new result to be appended to DOM
...
//Step 3 (getInspiration - tip users search) - using the JSON response, populate the relevant part of your HTML with the variable inside the JSON
